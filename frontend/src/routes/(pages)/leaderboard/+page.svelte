...
<div class="mx-auto px-4 py-6 md:p-10" style="max-width: 960px;">
  ...
  {:else}
    <div class="leaderboard-scroll rounded-lg mt-6 overflow-hidden" style="background: hsl(var(--pg-elevated)); border: 1px solid hsl(var(--pg-border));">
      <table class="w-full text-sm" style="border-collapse: collapse;">
        ...
        <tbody>
          {#each rows as r}
            <tr style="border-top: 1px solid hsl(var(--pg-border));">
              <td class="px-3 sm:px-4 py-3.5">
                {#if r.rank <= 3}
                  <Icon name="trophy" size={20} style="color: {trophyColors[r.rank - 1]};" />
                {:else}
                  <span style="color: hsl(var(--pg-fg-muted));">{r.rank}</span>
                {/if}
              </td>
              <td class="px-3 sm:px-4 py-3.5 font-mono text-[11.5px]" style="color: hsl(var(--pg-fg));">
