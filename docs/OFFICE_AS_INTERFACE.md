...
| the office survives ordinary accidents, 2026-08-07 | **reload mid-run**: the stream dies with the page and the task sat under DOING forever — now scrubbed back to `inbox` on load, keeping its assignee, the way `missionsOnLoad` has always handled a dead mission. **Letting a coworker go mid-task**: already clean — `abortAgentRun` kills the stream, the abort branch returns the task to inbox and clears the assignee, and a task still pointing at someone dismissed renders "unassigned · pick someone, then ▶ START" with the START button gated on the RESOLVED coworker, so there is no dead button to press |
